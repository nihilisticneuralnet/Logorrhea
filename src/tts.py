...
class TTSConfig:    
    def __init__(self,agent_1_voice: str = "af_bella",agent_2_voice: str = "am_adam",sample_rate: int = 24000,enable_realtime: bool = True):
...
class DebateTTSGenerator:
    def __init__(self,config: Optional[TTSConfig] = None,output_dir: str = "./audio_outputs"):
    ...
    def get_voice_for_agent(self,agent_id: Literal["agent_1", "agent_2"]) -> str:
        if agent_id == "agent_1":
            return self.config.agent_1_voice
        else:
            return self.config.agent_2_voice
    
    def generate_utterance_audio(self,text: str,agent_id: Literal["agent_1", "agent_2"],turn_number: int) -> np.ndarray:
    ...
    def add_utterance(self,text: str,agent_id: Literal["agent_1", "agent_2"],turn_number: int,add_pause: bool = True) -> None:
        audio = self.generate_utterance_audio(text, agent_id, turn_number)
        
        if len(audio) > 0:
            # uncomment below to save each utterance in real-time
            # rt_filename = f"realtime_turn_{turn_number:03d}_{agent_id}.wav"
            # rt_filepath = self.output_dir / rt_filename
            # sf.write(str(rt_filepath), audio, self.config.sample_rate)
            # logger.info(f" Real-time saved: {rt_filepath}")
    ...
    def save_debate_audio(self,filename: Optional[str] = None,topic: Optional[str] = None) -> str:
        if filename is None:
            if topic:
                safe_topic = "".join(
    ...
    def save_individual_utterances(self,prefix: str = "turn") -> List[str]:
        saved_files = []
